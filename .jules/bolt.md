...
## 2025-05-18 - [O(log N) optimizations for High Frequency Context Updates]
**Learning:** The `currentLineIndex` and `currentChapter` in `useAudioPlayer` update 4+ times per second via `currentTime`. An O(N) linear search over transcript lines block the main thread and scales poorly.
**Action:** When working with sequentially structured arrays (like timestamped transcript lines) and high-frequency state updates, always replace linear `Array.prototype.find` and `for` loops with O(log N) binary search patterns.
