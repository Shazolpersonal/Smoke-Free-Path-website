...
## 2024-09-06 - [High Frequency Array Traversal Avoidance in Chapters]
**Learning:** Similar to finding the active list element, finding the current chapter using `Array.prototype.find` repeatedly inside the `useMemo` block creates unnecessary CPU overhead on every audio tick (~4 times per second).
**Action:** When deriving state inside high-frequency update cycles like audio time updates, replace O(N) operations (e.g., `Array.prototype.find`) with O(log N) algorithms (e.g., binary search on a sorted array of timestamps). This prevents blocking the main thread as data scales.
