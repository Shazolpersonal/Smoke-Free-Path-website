...
## 2024-11-20 - [O(log N) Binary Search for High-Frequency Audio Updates]
**Learning:** The audio player context updates its state `~4 times per second` during playback. For longer tracks (and therefore larger transcripts), evaluating `currentLineIndex` and `currentChapter` by linearly scanning the array (O(N) operations) unnecessarily consumes CPU cycles on the main thread and can drop frames. Since transcript lines and chapters are chronologically ordered by time, an O(log N) binary search is much more efficient and eliminates a potential CPU bottleneck.
**Action:** When working with chronologically ordered array data (like transcripts, subtitles, or cue points) in high-frequency update loops (like audio/video timeupdate events), always use binary search (O(log N)) instead of linear search methods like `array.find()` or `for` loops.
