...
## 2024-08-10 - [O(log N) Binary Search for High-Frequency Loops]
**Learning:** In audio or video player contexts where a value (like current transcript line or active subtitle) needs to be constantly updated based on the current playback time (updating ~4x/sec), using an O(N) operation like a linear array search inside a `useMemo` block creates unnecessary CPU strain. The `currentLineIndex` linear search through hundreds of transcript lines wastes computation on every tick.
**Action:** Always favor O(log N) algorithms (like a binary search) for identifying active indices from sorted arrays (e.g. timestamps) when the search runs in a high-frequency update loop. This dramatically reduces the number of loop iterations needed to find the target.
