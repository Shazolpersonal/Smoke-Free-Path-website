...
## 2024-11-20 - [High Frequency Logic Optimizations (Linear vs Binary Search)]
**Learning:** During playback, `AudioProvider.tsx` computes the `currentLineIndex` about 4 times a second. Previously, it used a linear reverse search (`O(N)`) over the entire transcript array to find the current active line. Even with memoized child components, this CPU-intensive operation fired on every tick, creating a performance bottleneck in a high-frequency update loop.
**Action:** Always favor `O(log N)` algorithms, such as a binary search, when searching sorted data structures in high-frequency React context loops, as it eliminates unnecessary CPU overhead and reduces overall execution time significantly.
