...
## 2024-05-30 - [O(N) Operations in High-Frequency Context Updates]
**Learning:** The audio player context updates `currentLineIndex` and `currentChapter` about 4 times a second. Previously, these were found using an O(N) array scan. For long audio transcripts, iterating through the entire array linearly on every context update tick blocks the main thread and impacts playback smoothness.
**Action:** When deriving state from a frequently updated value (like audio `currentTime`) against a large, sorted array (like transcript lines or chapters), replace O(N) linear scans with an O(log N) binary search algorithm to ensure efficient execution and prevent UI stuttering.
