...
## 2024-06-25 - Transcript Index Binary Search
**Learning:** In audio playback contexts (`useAudioPlayer`), the `currentTime` updates frequently (~4x/sec), causing any derived state hooks to recalculate constantly. Using linear array searches (O(N)) for finding the current transcript line across large transcripts inside these high-frequency update loops causes unnecessary CPU overhead.
**Action:** Always favor O(log N) algorithms like binary search for finding chronological matches (like transcript lines or chapters) during high-frequency update cycles like `timeupdate` events.
