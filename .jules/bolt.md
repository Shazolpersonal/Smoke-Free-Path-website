...
## 2024-07-29 - [O(n) to O(log n) Transcript Search]
**Learning:** Frequent context updates (like the audio player's time updates ~4x/sec) can trigger performance bottlenecks when O(n) linear searches are executed during each state update to find the current active element. Since transcript lines are chronologically sorted arrays, this is an avoidable bottleneck.
**Action:** When a continuous state change (like a video/audio play head) must match to a chronologically sorted data array (like transcript lines or subtitles), always implement a binary search algorithm (O(log n)) instead of a linear search to find the active element to prevent blocking the main thread during high-frequency renders.
