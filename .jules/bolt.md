...
## 2024-08-01 - [High Frequency Search in Context Arrays]
**Learning:** During continuous playback, `AudioProvider` recalculates active chapters and active lines multiple times per second on every audio `timeupdate`. Linear operations like `Array.find` or `for` loops create an O(N) overhead that scales poorly as content grows, causing needless CPU usage. Since the data sets (chapters, lines) are chronologically ordered, they natively support O(log N) lookups.
**Action:** Replace linear search patterns (`Array.find` or backwards linear iteration) with binary search implementations for any state variables re-computed heavily inside a high-frequency event listener (like `timeupdate` or `scroll`), assuming the collection is appropriately sorted.
