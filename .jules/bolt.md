...
## 2024-08-01 - [AudioProvider transcript synchronization calculation]
**Learning:** The audio player frequently (~4x/sec) re-calculates the active `currentLineIndex` based on `currentTime`. Utilizing a linear search algorithm backwards through the transcript array results in an O(N) operation running on every tick.
**Action:** Always replace O(N) lookup operations running within high-frequency update loops (like audio/video `timeupdate` events) with O(log N) approaches, like a binary search, especially for ordered sequential data like transcript timestamps.
