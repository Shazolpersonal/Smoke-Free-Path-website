...
## 2024-08-01 - [Binary Search for Frequent Playback Time Synchronization]
**Learning:** In audio or video players where playback time updates frequently (e.g., ~4x/sec), looking up the active chapter or line index via linear searches (`Array.find()` or looping backwards) inside `useMemo` scales poorly (O(N)) as the transcript or chapter list grows. This unnecessary CPU overhead during rapid event firing can drop frames.
**Action:** Always prefer binary search (O(log N)) when searching sorted time-series arrays (like timestamps) during continuous playback events. This guarantees microsecond-level performance even on massive transcripts.
