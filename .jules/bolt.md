...
## 2024-11-20 - [Optimize High-Frequency Data Lookups]
**Learning:** For components that re-calculate derived state frequently based on a time value (like an audio player seeking through transcript lines ~4 times a second), linear scans (O(N)) through large arrays can quickly become a performance bottleneck as the data size grows.
**Action:** When searching chronologically ordered data (like timestamps) during a high-frequency update cycle (e.g. `requestAnimationFrame` or `timeupdate` events), always implement a binary search algorithm (O(log N)) to reduce CPU overhead and keep the main thread free.
