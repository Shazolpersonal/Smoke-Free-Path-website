...
## 2024-08-01 - [O(log N) Binary Search for High-Frequency State Updates]
**Learning:** Frequent context updates (such as those triggered by the audio player's `timeupdate` event, which fires multiple times per second) recalculate derived state based on arrays of data (e.g., matching a current time to a transcript line or chapter). A linear search `.find()` or backward `for` loop is $O(N)$ and causes unnecessary CPU overhead, especially as transcript sizes grow.
**Action:** When a context provides frequently updating time-based values that map to a sorted array (like `transcript.lines` by time), always use binary search algorithms $O(\log N)$ instead of linear scans $O(N)$ to minimize calculation time during the browser's render loop.
