...
## 2026-06-13 - [Optimize Frequent Context Lookups to O(log N)]
**Learning:** The application's audio context (`useAudioPlayer`) triggers updates ~4x/sec, meaning any derived state calculations (e.g., finding the `currentChapter` or `currentLineIndex`) run continuously. Using O(N) Array methods like `find()` or reverse iteration over hundreds of transcript lines blocks the main thread and introduces unnecessary CPU overhead during playback.
**Action:** When working with frequently updated contexts and ordered data (like timestamps), replace O(N) operations with O(log N) algorithms (e.g., Binary Search). Always ensure that calculations within high-frequency update loops are as efficient as possible to prevent UI jank.
