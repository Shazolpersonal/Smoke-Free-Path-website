...
## 2024-05-28 - [Avoid O(N) searches inside high-frequency React hooks]
**Learning:** The project's audio context (`useAudioPlayer`) updates the `state.currentTime` roughly 4 times per second. Calculating the active chapter and current transcript line via `O(N)` linear searches (e.g., `Array.prototype.find()` or backward `for` loops) causes unnecessary CPU overhead that compounds with transcript size, potentially leading to frame drops during continuous playback.
**Action:** When deriving values from ordered lists (like timestamps) inside continuously updating hooks, always use `O(log N)` algorithms like binary search rather than `O(N)` linear searches to maintain consistent 60FPS performance.
