...
## 2026-06-24 - [O(log N) Binary Search for High Frequency Update Cycles]
**Learning:** During audio playback, the current transcript line index needs to be recalculated constantly as the audio time progresses (~4x/sec). Utilizing a linear `O(N)` loop to match the playback time with a specific transcript line degrades performance slightly for large transcript files. Because transcript lines are sequential and strictly ordered by time, it is highly suitable for an `O(log N)` binary search.
**Action:** Replace `O(N)` linear searches with `O(log N)` binary search strategies when mapping timestamps to strictly ordered arrays during high frequency event loops (such as audio/video playback time updates).
