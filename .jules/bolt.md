...
## 2026-08-29 - [Optimizing High-Frequency Data Lookups]
**Learning:** During playback, the audio context updates the `currentTime` very frequently (~4 times a second). Any `useMemo` hooks depending on `currentTime` that iterate over long data arrays (like searching for the current chapter or line using `Array.prototype.find()` which is O(N)) execute on every update, unnecessarily consuming CPU cycles.
**Action:** Always replace O(N) linear searches with O(log N) binary searches in high-frequency context values when searching through ordered arrays (like transcript lines based on timestamps). This drastically reduces JS main thread execution time during continuous updates.
