...
## 2024-07-29 - [O(log N) Binary Search for High-Frequency State Updates]
**Learning:** Frequent context updates (e.g., audio time updates ~4x/sec) combined with O(N) array scanning (like finding the current transcript line via a linear backward loop) can create CPU overhead and UI stuttering as transcript length grows.
**Action:** When a high-frequency interval updates state that depends on finding an element in a sorted list (e.g., timestamps in a transcript), always use an O(log N) algorithm like binary search rather than O(N) linear scanning.
