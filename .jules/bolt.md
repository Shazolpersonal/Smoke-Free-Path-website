...
## 2026-07-08 - [O(log N) Binary Search on Hot Paths]
**Learning:** The project's audio context (`useAudioPlayer`) executes derivations like `currentLineIndex` very frequently (~4x/sec) based on `state.currentTime`. Using linear O(N) operations (like `Array.prototype.find` or `for` loops) on large arrays (e.g. transcript lines) inside these hot paths causes unnecessary CPU overhead. Since the arrays (chapters, transcript lines) are sorted chronologically by nature, they are perfect candidates for binary search.
**Action:** Avoid linear array searches (O(N)) during high-frequency update cycles (like audio/video `timeupdate` events). Instead, favor O(log N) approaches like binary search when operating on sorted data.
