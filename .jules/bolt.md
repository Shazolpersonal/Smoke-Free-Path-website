...
## 2024-07-29 - [Binary Search for Ordered Time-Series Data]
**Learning:** Frequent context updates (like audio playback time triggering ~4x/sec) that scan an ordered list of items (like transcript lines or subtitles) can cause noticeable CPU overhead and dropped frames if the search is O(N) linear scan, particularly on slower devices.
**Action:** When searching chronologically ordered time-series arrays inside a frequent update cycle (e.g. `requestAnimationFrame` or `timeupdate`), always replace the linear scan with an O(log N) binary search.
