...
## 2024-11-04 - [Time-Series Array Lookups on High Frequency Ticks]
**Learning:** The audio player's `timeupdate` event fires frequently (~4x/sec), triggering a search to identify the active `currentLineIndex`. Doing a linear `O(N)` loop on an ever-growing array of transcript times can bottleneck the main thread when executed so rapidly, even inside a `useMemo` block.
**Action:** Always replace `O(N)` linear array searches with `O(log N)` binary searches when identifying indices in sorted time-series data during high-frequency update loops (e.g. `timeupdate` or `requestAnimationFrame`).
