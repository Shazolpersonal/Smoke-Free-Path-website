...
## 2026-07-03 - [High-Frequency Updates and Search Complexity]
**Learning:** High-frequency event updates (like `timeupdate` firing ~4x/sec during audio playback) can strain performance when O(N) operations are performed in their dependency chains. In this codebase, the `useAudioPlayer` context computed `currentLineIndex` linearly over a potentially long transcript array.
**Action:** Always favor O(log N) algorithms (e.g., binary search) over O(N) linear operations for derived state computed inside high-frequency update loops (e.g. `useMemo` reacting to `state.currentTime`), especially when dealing with ordered data like transcript timestamps.
