...
## 2024-11-20 - [O(log N) Binary Search for Ordered High-Frequency Data]
**Learning:** When data arrays are chronologically ordered (e.g. transcript timestamps) and are checked within high-frequency hooks (like `timeupdate` updating 4x/second), using O(N) linear search can build up unnecessary CPU overhead, especially with long array lengths.
**Action:** Replace O(N) iterations with O(log N) binary search algorithms when calculating states based on ordered thresholds. This pattern dramatically cuts iterations and prevents potential main thread blocking.
