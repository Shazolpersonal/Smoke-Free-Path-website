...
## 2024-08-01 - [O(log N) searches in Audio Context]
**Learning:** Frequent context updates within media playback can result in O(N) operations being executed rapidly (e.g., matching a line in a transcript using `.find()`). As the size of lists like transcripts increases, this creates a performance bottleneck leading to lag during playback.
**Action:** When working with chronologically ordered lists and needing to find items based on time intervals, replace O(N) linear operations like `.find()` or `for` loops with O(log N) binary search algorithms for performance optimizations.
