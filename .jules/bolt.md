...
## 2024-07-02 - Audio Player Transcript Re-render Optimization
**Learning:** Audio progress states (like `state.currentTime`) update frequently (often ~4x/sec). Calculating derived state (like current transcript line or chapter) using linear O(N) operations inside these frequent hooks can cause unnecessary CPU spikes, especially when combined with large transcript arrays.
**Action:** When deriving state from frequently updated timing values (like audio or video playback), always use O(log N) algorithms like binary search instead of `find()` or linear backwards scanning.
