...
## 2024-07-29 - [Optimizing High-Frequency React Context Derived State]
**Learning:** The audio player context computes derived state (`currentLineIndex`, `currentChapter`) on every `timeupdate` tick (~4x/sec). Previously, it used `O(N)` linear searches over the transcript arrays. Since these operations run very frequently and the arrays are pre-sorted by time, the linear search unnecessarily consumed CPU cycles.
**Action:** When computing derived state that runs frequently (like within an audio update loop), replace `O(N)` linear array operations (e.g., `find()`, `findIndex()`, `reverse()`, loop iterations) with `O(log N)` binary search algorithms, provided the data is already sorted.
