...
## 2024-08-01 - [Optimizing High Frequency Time-Based Lookups]
**Learning:** In contexts where an array of time-stamped events (like transcript lines) is searched repeatedly on a high-frequency tick (e.g., `timeupdate` firing 4+ times a second), linear searches (O(N)) become a noticeable CPU overhead, especially as the array size grows.
**Action:** When searching a sorted array on a hot path, always favor a binary search (O(log N)). It provides mathematically guaranteed better performance and eliminates a hidden, scaling bottleneck.
