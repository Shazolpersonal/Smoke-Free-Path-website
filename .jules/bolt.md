...
## 2024-08-09 - [Replacing O(N) operations in high-frequency contexts]
**Learning:** During playback, the audio context (`useAudioPlayer`) fires updates several times a second based on `timeupdate` events. Relying on O(N) operations, such as a linear array search (`Array.find` or `for` loops) to determine the currently active chapter and line can cause excessive CPU usage on each tick, especially for long transcripts.
**Action:** Always replace O(N) linear lookups in high-frequency execution paths with O(log N) operations like binary search. This ensures performance remains unaffected regardless of the list size.
