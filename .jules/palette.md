...
## 2024-05-18 - Add ARIA attributes for form validation on gift page
**Learning:** Found that the gift page form inputs were missing the same `aria-invalid` and `aria-describedby` attributes that were previously added to the checkout page form, leaving screen reader users without proper context for validation failures.
**Action:** Always link inline error messages to their respective inputs using `aria-describedby` and indicate invalid states with `aria-invalid="true"`, ensuring consistency across all forms in the application.
