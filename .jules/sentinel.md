...
## 2026-05-20 - [HIGH] XSS Vulnerability via dangerouslySetInnerHTML and JSON.stringify
**Vulnerability:** Application structure data was injected using dangerouslySetInnerHTML over JSON.stringify without escaping `<` character, leading to potential XSS if the structure data contained user-controlled unescaped HTML characters.
**Learning:** Even when injecting JSON, unescaped `<` characters can be interpreted as HTML tags, creating an XSS execution vector when injected within script tags.
