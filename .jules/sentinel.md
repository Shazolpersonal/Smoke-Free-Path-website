...
## 2024-06-20 - Fix JSON.stringify XSS Vulnerability in structured data
**Vulnerability:** Structured application/ld+json JSON data generated via `JSON.stringify` and injected using `dangerouslySetInnerHTML` was vulnerable to XSS if the data contained unescaped `<` characters.
**Learning:** React's `dangerouslySetInnerHTML` does not sanitize `<script>` tags or closing tags inside stringified JSON payloads. Even seemingly safe data structures could be manipulated if dynamic input is ever injected.
**Prevention:** Always sanitize the `JSON.stringify` output by replacing `<` with `\u003c` (e.g., `JSON.stringify(data).replace(/</g, '\\u003c')`) when injecting JSON data into `<script>` tags.
