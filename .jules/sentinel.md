...
## 2024-05-24 - [CRITICAL] Prevent XSS in JSON-LD `dangerouslySetInnerHTML`
**Vulnerability:** Unescaped `<` characters in `JSON.stringify` output when passed directly to `dangerouslySetInnerHTML` for structured data (`application/ld+json`). If user input is somehow reflected in the schema, an attacker can prematurely close the `<script>` tag and execute arbitrary code.
**Learning:** React's `dangerouslySetInnerHTML` does NOT sanitize its contents. Injecting `JSON.stringify(data)` directly into a `<script>` tag within React is a common but dangerous pattern because JSON serialization does not escape the `<` character by default.
**Prevention:** Always sanitize the `JSON.stringify` output by explicitly escaping the `<` character to its Unicode equivalent: `.replace(/</g, '\\u003c')`.
