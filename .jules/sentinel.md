...
## 2025-02-14 - Fix XSS Vulnerability in JSON structured data
**Vulnerability:** Unescaped `<` characters in JSON structured data strings when injected into `<script>` tags using React's `dangerouslySetInnerHTML`.
**Learning:** `JSON.stringify()` output alone is not inherently safe for `<script>` injection. An attacker could embed `</script><script>alert(1)</script>` inside JSON fields to breakout of the structured data block and execute malicious scripts.
**Prevention:** Always escape the `<` character as `\u003c` in `JSON.stringify()` output using `.replace(/</g, '\\u003c')` when injecting it as innerHTML inside script tags.
