...
## 2026-05-20 - [HIGH] XSS via unescaped HTML in JSON injection
**Vulnerability:** Unsanitized JSON stringified data was injected into script tags using `dangerouslySetInnerHTML`.
**Learning:** Using `JSON.stringify` inside `dangerouslySetInnerHTML` without escaping the `<` character exposes the application to XSS execution vulnerabilities if the JSON contains HTML tags, since the browser will interpret `</script>` or other tags within the JSON payload.
**Prevention:** Always sanitize JSON output injected into script tags by replacing `<` with `\u003c`.
