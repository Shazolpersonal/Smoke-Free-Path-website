...
## 2026-05-19 - [HIGH] XSS Vulnerability via JSON in Script Tags
**Vulnerability:** Structured data (application/ld+json) was injected into script tags using `dangerouslySetInnerHTML={{ __html: JSON.stringify(data) }}`. If the JSON data contained unescaped HTML characters, specifically `</script>`, it could break out of the script context and execute arbitrary XSS payloads.
**Learning:** `JSON.stringify` does not escape HTML characters like `<` by default, making it unsafe to inject directly into HTML contexts without further sanitization.
**Prevention:** Always sanitize the output of `JSON.stringify` when injecting into HTML by replacing `<` with its unicode escape sequence `\u003c` (e.g., `JSON.stringify(data).replace(/</g, '\\u003c')`).
