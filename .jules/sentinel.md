...
## 2026-05-20 - [HIGH] XSS Vulnerability in JSON Script Tags
**Vulnerability:** Several places in the app injected structured data (`application/ld+json`) by calling `JSON.stringify` directly within `dangerouslySetInnerHTML`. Because JSON can contain HTML entities (like `<script>...</script>`), failing to escape these characters permits arbitrary script execution if the JSON source is user-controllable.
**Learning:** `JSON.stringify` alone is NOT safe for direct injection into a script tag context.
**Prevention:** Always sanitize the `JSON.stringify` output when injecting into HTML by explicitly escaping angle brackets using `.replace(/</g, '\\u003c')`.
