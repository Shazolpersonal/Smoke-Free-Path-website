...
## 2026-05-20 - [HIGH] XSS Vulnerability in JSON Structured Data Injection
**Vulnerability:** JSON-LD structured data was injected into script tags using dangerouslySetInnerHTML without escaping HTML characters, allowing potential XSS via unescaped '<' characters.
**Learning:** JSON.stringify does not escape HTML tags by default. If the JSON data contains user input or malicious strings with '<script>', it can break out of the script tag context and execute.
**Prevention:** Always sanitize JSON.stringify output by replacing '<' with '\u003c' when injecting into HTML.
