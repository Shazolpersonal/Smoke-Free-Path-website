...
## 2024-05-20 - [HIGH] XSS Vulnerability in dangerouslySetInnerHTML JSON injection
**Vulnerability:** The application used `dangerouslySetInnerHTML={{ __html: JSON.stringify(data) }}` to inject structured JSON data into `<script>` tags. Without sanitizing the JSON output, if `data` contained an unescaped `<script>` closing tag or other HTML characters, it could lead to Cross-Site Scripting (XSS) execution vulnerabilities.
**Learning:** `JSON.stringify` does not escape HTML characters like `<` or `>`. When its output is placed inside an HTML context (like a `<script>` tag), the browser interprets these characters, potentially closing the current tag and executing malicious code.
**Prevention:** Always sanitize the `JSON.stringify` output by replacing `<` with `\u003c` when injecting JSON data into `<script>` tags using `dangerouslySetInnerHTML`.
