...
export function AudioProvider({ children }: { children: ReactNode }) {
  ...
  // ⚡ Bolt Optimization:
  // Problem: Linear search for currentChapter runs multiple times per second
  // during audio playback (due to state.currentTime updates), creating O(N) overhead.
  // Solution: Replace Array.prototype.find() with binary search O(log N) since chapters are sequential.
  // Impact: Reduces CPU overhead on high-frequency state updates, preventing main-thread blocking.
  const currentChapter = useMemo<TranscriptChapter | null>(() => {
    ...
    while (left <= right) {
      ...
      if (state.currentTime >= chapter.start && state.currentTime < chapter.end) {
        return chapter;
      } else if (state.currentTime >= chapter.end) {
        left = mid + 1;
      } else {
        right = mid - 1;
      }
  ...
  const currentLineIndex = useMemo<number>(() => {
    ...
    // ⚡ Bolt Optimization:
    // Problem: The previous linear search O(N) from the end of the array ran
    // ~4 times per second during playback, creating unnecessary CPU overhead
    // as the transcript size grows.
    // Solution: Replaced with a binary search O(log N).
    // Impact: Reduces time complexity from O(N) to O(log N), keeping the main
    // thread unblocked during high-frequency audio context updates.
    let left = 0;
    ...
    while (left <= right) {
      ...
      if (state.currentTime >= lines[mid].t) {
        bestIndex = mid;
        left = mid + 1; // Try to find a later line that also started
      } else {
        right = mid - 1;
      }
    }
