...
export function AudioProvider({ children }: { children: ReactNode }) {
  ...
  const currentLineIndex = useMemo<number>(() => {
    ...
    // ⚡ Bolt Optimization: Binary search replaces O(N) reverse linear scan.
    // Time updates ~4x/sec; with hundreds of transcript lines, O(N) causes CPU overhead.
    // Binary search (O(log N)) instantly finds the current line, drastically improving performance.
    let left = 0;
    ...
    while (left <= right) {
      ...
      if (lines[mid].t <= state.currentTime) {
        best = mid; // this is a candidate, look for later ones
        left = mid + 1;
      } else {
        right = mid - 1;
      }
