...
export function AudioProvider({ children }: { children: ReactNode }) {
  ...
  const currentChapter = useMemo<TranscriptChapter | null>(() => {
    ...
    // ⚡ Bolt Optimization:
    // O(log N) binary search instead of O(N) linear scan to find the current chapter.
    // Audio time updates frequently, so finding the active element needs to be fast.
    while (left <= right) {
  ...
  const currentLineIndex = useMemo<number>(() => {
    ...
    // ⚡ Bolt Optimization:
    // O(log N) binary search instead of O(N) reverse iteration to find the current line.
    // Audio time updates ~4x/second. Finding the active line amongst hundreds of lines
    // needs to be optimally fast to prevent main thread blocking during playback.
    while (left <= right) {
      const mid = Math.floor((left + right) / 2);
